...
而光子到达相机时，不会附带标签，告诉相机自己属于哪一项。

那我们为什么还能从太阳光推断温度？

## 黑体：先构造一个只由温度决定的发光体

为了从光反推温度，最麻烦的不是公式，而是影响光的因素太多。
...
如果材料、反射、透射和发光机制都未知，仅凭“看起来很红”显然无法唯一确定温度。

物理学中常用的办法，是先构造一个理想对象，把这些复杂因素尽可能拿掉。这个理想对象就是**黑体（blackbody）**。
...
反过来，如果我们确认一个对象足够接近黑体，并且准确测到了它的辐射光谱，就可以根据光谱反推温度。

真实物体通常不是理想黑体。它在某个波长上的热辐射常被粗略写成：
...
### Wien 位移定律：越热，峰值越靠近短波

从 Planck 曲线可以进一步得到 Wien 位移定律：
...
也就是说，温度越高，黑体谱辐射亮度峰值对应的波长越短。

它很好地解释了日常经验：

- 温度较低的高温物体先在红外中变亮；
- 继续升温后开始暗红；
- 再升温会出现橙色、黄色，并逐渐包含更多短波可见光。

对大约 $5800\ \mathrm K$ 的黑体，按波长表示的峰值在约 $500\ \mathrm{nm}$ 附近，落在可见光范围内。这提供了理解太阳颜色和温度关系的第一层直觉。

但 Wien 位移定律只利用了峰值位置。真实测量不会因为相机里最亮的是绿色，就直接代入一个公式得到太阳温度。太阳不是完美黑体，光谱中还有大量吸收线；地球大气和仪器也会改变测得的光谱。更可靠的做法是测量更完整的光谱或总辐射量，再结合仪器标定和物理模型拟合。
...
| 定律                  | 回答的问题                         |
| --------------------- | ---------------------------------- |
| Planck 定律           | 每个波长上分别发出多少辐射？       |
| Wien 位移定律         | 谱辐射亮度峰值大致在哪里？         |
| Stefan–Boltzmann 定律 | 全部波长加起来，总共发出多少辐射？ |
...
<img class="theme-adaptive-diagram" src="/images/posts/how-light-reveals-temperature/camera-imaging-pipeline.svg" alt="从场景辐射经过光学系统、传感器、采集转换和计算摄影，最终得到RAW或JPEG、HEIF图像的相机成像链路" />
...
> 用手机拍一张蜡烛火焰的照片，能不能根据颜色算出火焰温度？

如果只是普通自动模式拍摄的一张 JPEG，答案是：**完全不能可靠地得到蜡烛火焰的绝对温度。**
...
所以在问“蜡烛火焰多少度”之前，至少还要问：

> 哪个位置？哪种物质？通过什么观测定义得到的温度？
...
太阳并没有固体表面。我们能看到光球层，是因为更深处的太阳物质对可见光足够不透明，深层光子难以直接逃逸；更外层又逐渐变得透明。我们看到的是光能够有效逃逸出来的一段区域，而不是一堵坚硬的墙。

蜡烛火焰与太阳当然不是同一个尺度、同一种介质，但“观测波段下的光学厚度决定我们主要看见哪一层”这个思路是相通的。
...
一台相机丢失了视线方向上的深度信息。那从不同方向多放几台相机，似乎就能把深度找回来。

这个思路是合理的。不同方向的视线穿过火焰的路径不同，多视角观测确实能增加三维空间约束。
...
这些约束能让结果稳定，却也意味着三维图中不只有“相机看到的信息”，还包含了人为选择的先验假设。

因此，一张平滑、漂亮、可以旋转的三维云图，并不能单独证明它就是唯一真实的三维温度场。至少还要检查：

- 用这份三维结果重新预测各台相机图像，是否与真实观测一致？
- 留出一台未参与重建的相机，能否正确预测它看到的图像？
- 改变少量噪声、机位和参数后，结果是否稳定？
- 是否有热电偶、光谱测温或其他独立仪器进行验证？
- 图中显示的是热力学温度、辐射温度，还是等效发光强度？
...
可以进一步估计火焰中烟炱辐射温度、发光分布，甚至尝试重建三维温度相关场。但结果的含义、模型依赖和不确定度必须明确，不能因为输出文件的字段叫 `temperature`，就认为它已经等同于每个空间点的真实热力学温度。

不论使用哪种方案，通过光测温都是这个整体流程：

<img class="theme-adaptive-diagram" src="/images/posts/how-light-reveals-temperature/temperature-inference-chain.svg" alt="通过光测温的五步证据链：定义温度、明确发光和传播机制、标定相机与完整光路、根据模型反推温度、用独立数据验证结果" />
