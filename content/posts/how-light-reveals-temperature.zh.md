...
我们能看见房间里的一张红色桌子，通常不是因为桌子热得发出了红光，而是因为：
...
可以这样来简单理解：温度越高，系统中热运动的典型能量尺度 $k_{\mathrm B}T$ 越高，因此能量较高的光子也更容易被热激发。光子的能量满足：

$$
E_{\mathrm{photon}}=h\nu
$$

所以光子的能量越高，对应的频率 $\nu$ 也越高。黑体辐射中，光子的平均能量与温度成正比：
...
更准确点的说法：物质通过碰撞、带电粒子运动以及原子和分子的不同能级过程与电磁场交换能量，最终形成由温度决定的（也不是温度决定，而是温度就是这么定义的）热平衡辐射。至于峰值的准确位置，还同时取决于不同频率下的电磁场模式数量和光子的统计分布。
...
RAW 数字值
    ↓ 去马赛克、白平衡、颜色矩阵、降噪和锐化
    ↓ Gamma/传递函数、Tone Mapping、HDR 与压缩
JPEG、HEIF 编码
```
...
颜色变化
可能来自温度变化
也可能来自发光物种、颗粒浓度和反应过程变化
```

如果事先证明某一区域主要由高温烟炱连续热辐射主导，再使用经过标定的窄波段或多光谱仪器，确实可以估计烟炱的辐射温度或色温。但这个结论不能自动推广成“手机拍一下，就得到了整个火焰中气体的热力学温度”。

## 固体表面和半透明火焰不是同一种成像问题

测量一个不透明固体表面时，在几何光学近似下，一条相机视线通常会落到表面的某个位置。只要路径中的空气相对透明，像素主要对应这个表面小区域发出和反射的辐射。

火焰却不是一张二维表面，而是有厚度的三维介质。相机的一条视线可能依次穿过：
...
每一段都可能自己发光，也可能吸收前面或后面传来的光。于是一个像素记录的不是某个空间点，而更接近：
...
我们之所以能通过光推断太阳光球层的有效温度，不是因为太阳看起来“像某种颜色”，而是因为：

1. 太阳的连续辐射背景可以在较大波段内近似为黑体辐射；
2. Planck 定律给出了温度与完整光谱之间的关系；
3. Wien 位移定律提供了峰值随温度移动的直觉；
4. Stefan–Boltzmann 定律连接了温度与总辐射功率；
5. 太阳光谱、总辐照度和光球层半径可以被独立测量；
6. 仪器响应、观测条件和温度定义都相对明确。

而面对一张手机拍摄的蜡烛照片，我们缺少的恰好是这条链路中的许多环节：
...
所以，通过光测温并不是简单地“根据颜色查温度”，而是一个完整的推断过程：
