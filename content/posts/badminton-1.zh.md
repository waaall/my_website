...
date: 2024-04-23
tags: [羽毛球, 运动]
summary: 浅谈羽毛球：球拍、握拍、挥拍、步伐等。
---

## 前言

参考：[Badminton - Wikipedia](https://en.wikipedia.org/wiki/Badminton#External_links)

### 基本球路
基本球路包括：高远（双方底线之间）、吊球（后场到对方网前）、杀球（向下重击）、平抽（中场快速对抗）、放网（己方网前回到对方网前）、挑球（己方网前到对方底线）等。
...
这里的 Is 即为以握把端点为轴、绕该轴的转动惯量（即俗称的"挥重"）。
...
### 1.1正手握拍
李宗伟、林丹式（半拳式）的关键特征：拇指和食指轻贴在拍柄相邻的两个宽面上形成"V 形"，其余三指自然收拢但不抓死，掌心与手柄之间留有空隙。

#### 正手握拍注意事项
首先一定是半拳式。为什么不是拳式李雨轩的视频中讲到了。但是有些球员在杀球的尾段动作看起来是拳式，其实也不是，只是半拳式握紧之后的状态——拇指和食指几乎抬到一个高度，很像拳式，但并不是。

典型的拳式错误是：四指完全握拢成拳，拇指压在食指外侧，整个手掌"抱"住手柄，掌心紧贴手柄不留空隙。这种拳式的问题不用我再阐述了，最好还是半拳式，可以在握紧之后像林丹一样拇指提上来一点，便于握紧。
...
### 架拍
新手最常见的错误是两个大臂都"沉下去了"，肘部低于肩膀，这样不利于力传导。架拍过程中持拍手沉下一点可以接受，但是引拍前要提上来，也就是刘辉教练说的提升框架。

参考李宗伟的动作：架拍瞬间肘部可以略低、放松；但在真正引拍之前，持拍手会主动抬起，最终持拍肘、持拍手、肩膀基本处在一条水平线上，再开始转体引拍。
### 关于侧身
...
### 3.0 步伐选择的整体逻辑
进入具体的启动、网前、后场步伐之前，先讲清楚一个比"怎么跑"更上层的问题：**应该跑去哪？什么时候跑？** 这是绝大多数业余球员（包括过去的我）想错的地方。

#### 3.0.1 第一性原理
羽毛球 = 在场上把自己尽量舒服地跑到接球位置，并把球打到对面让他不舒服。仅此而已。所以步伐就两个目的：
1. 服务于"接到球"；
2. 服务于"接到之后有发力空间"。

第二点常被业余忽视。比如被动接杀蹲下勉强够到球但身体完全失重，技术上算"接到了"，但下一拍必死，那这一拍的步伐是不合格的。

#### 3.0.2 回位位置不是固定的"中点"
新手（包括一年内的我）最大的误区就是：打完球必须回到几何中点。这是错的。

正确的逻辑是：**你回的球决定了对面下一拍的可选项，可选项决定了威胁分布，威胁分布决定了你的最佳站位。**

换句话说，只需要满足一条："不能存在对面能打出但你跑不到的球"。仅此一条原则就能推出绝大多数所谓"步伐套路"。比如：
- 你打了一个非常贴网的球，对方推不了（推必下网，原因下一节解释）、杀不了、扑也很难，几乎只能挑或回放。挑的话弧度高、时间充足；回放距离近但时间极短。所以站位要靠到白线附近抓回放，对方挑你也来得及退。
- 你打了一个不那么贴网的"远网球"，对方可以平推压你后场，弧度低、时间短，所以你不能再贴前站，必须退一些抓推（这就是"远网抓推"成立的物理基础）。
- 你回了一个高远，对方可以杀、吊、平高。威胁顺序是杀 > 吊 > 平高，所以"中"应该偏后一点（站到能第一时间防杀的位置），而不是几何中点。

#### 3.0.3 近网球 vs 远网球：一个常被忽略的本质差别
这点很多教学不讲清楚，但它是步伐选择的物理底层：

- **近网球**（贴网过去的）：对方想"推"是不可能的，球在网根附近，推出的轨迹必然下网。所以对方只剩三个选择——挑（弧度高、时间充足）、回放（距离近但球速慢）、勾对角，三种球都给你不算糟糕的反应时间。
- **远网球**（过网后落点离网较远）：球高度足够，对方可以打出低弧度的推球，球速快、过网时间极短，直接压你反应时间。

所以"贴不贴网"对你下一拍处理时间的影响是**质变**而不是量变。

这里有一个反直觉的推论：**从底线打回的近网球，必然是球速慢的球**。因为球要从底线落到近网而不下网，弧度必须够，球速一快就飞过去了。所以即使球离你很远，反应时间反而比想象中更充足。真正威胁大的球不是"近"，是"既能过网又球速快"，那就是杀球和吊球。

#### 3.0.4 时间预算：对手触球到球落地的窗口
上面所有结论的硬约束：

```
你的总时间预算 = 对手触球瞬间 → 球落到你这边的瞬间
这段时间内你必须完成：回动重心 → 看对方出手 → 启动 → 跑到位
此外还要给击球本身留出发力时间
```

所以"回中"不是越居中越好，而是在窗口内**能到得了威胁最大的球**就够了。窗口短的时候（比如吊球后），根本来不及跑回几何中点再启动，必须边回动边看出手，甚至没到中场就要在路上启动方向。

3.3.1 节（回中与启动的时机）那条经验——高远回两步、劈吊放网回一步——本质就是窗口长短不同。

#### 3.0.5 吊球：一个被低估的高风险球
吊球时间窗短，且你刚做完下手球动作，重心和姿势都不在最佳启动状态。如果掉到对方手里（人就站在那），等于单方面送分；即使掉到对方相对被动的位置，也只允许你回一步就要看出手。

更关键的是：业余最常见的死法是**吊完往前补位被推直线**。因为吊完之后人是顺势往前流动的，重心已经向前，这时对方一拍推直线，你要再把重心扭回往后场跑——重心转换是步伐里最贵的一件事，一来一回基本必死。所以滑板吊对角看起来威胁大，自己也很容易把自己吊死。

实战处理：吊球后宁可少回半步，先把重心稳住、看出手，再决定往哪去。

#### 3.0.6 一句话总结
打完球第一时间回收重心、做好启动姿势，然后**根据自己球的质量酌情决定回到多"中"**，并在路上看对方出手随时调整方向。这个原则比任何具体的"标准回中位置"都更接近真相。后面启动、网前、后场每一节，基本都是这个原则在不同球路下的具体落地。
...
##### 1. 弓箭步动作
要弯腿网前迈出去，重心压下去，同时非持拍手保持平衡。标准动作（如李宗伟示范）的关键特征：迈出腿弯曲且**膝盖不超过脚尖**，重心压在前腿大腿肌肉上而非膝盖。新手的典型错误是迈得不够远，导致够球时膝盖超过脚尖，重心直接压到膝盖上，承受巨大压力。

而即使做出了标准动作，如果股四头肌力量支撑不住、出现不稳定，也会损伤膝盖甚至脚踝；但这是做出动作之后的事——绝大多数初学者根本还没到这一步。如果打球时动作没问题、但这个动作不能稳定，那肯定还是要加强股四头肌力量。
##### 2. 回收重心
非持拍手和对应腿要起到很关键的作用，比如对于右手持拍的球员，左手要动态的抬高下压或者后摆来控制重心稳定，左腿要适当回收来避免靠右腿单腿力量"刹车"甚至回位。

新手的典型错误是：后腿留在原地"拖着"没有跟收，且非持拍手没有抬起平衡重心，整个人靠前腿单腿"挂"在弓箭步上，回不来。（左手球员就反过来）
...
因为前面已经把握拍、挥拍发力、步伐等基本功的基本功讲完了，接下来的练习方法主要是指的单一技术的训练，具体见[[羽毛球训练规程]]。
...
2. 对于被前后左右吊过多次的新手，终于学到了要回中的意识，这时候就可以重复点了,效果极佳，但是停顿肯定无效。
...
#### 5.1.1 发球与接发的位置
单打发球方在规则上是**天然的弱势方**。为什么？发球区被规则限制——只能落在罚球线之内的那一小块区域，不能发近网。而接发方却可以从这一小块区域里的来球回出全场任何角落。也就是说：

- 发球方的可选项被规则压缩到很小；
- 接发方的可选项几乎是全场。

所以接发回来的球往往比发球本身更主动，发球方处理接发的几拍是单打里非常吃亏的。所谓"赢的发球"，物理意义就在这。

**单打发球站位**
基于上面的弱势，发球站位要同时覆盖接发方最可能的两类回球：
1. 接发回放近网（球速慢但距离近，留给你的时间极短）；
2. 接发推后场（球速快、弧度低，压反应时间）。

所以发球应站在罚球线**往后一步左右**：
- 一个跨步上前能接到回放；
- 退一步能处理被推到后场的球。

很多业余球员发球贴在罚球线甚至更前，这是双打站位思路（双打有同伴补后场），单打这么站对方一推就死。

**发了小球之后**
发了小（近网）球后，接发方主动权很大，能选：
- 推（这是你最难处理的，弧度低时间短）；
- 放近网或勾对角（球速慢但距离近）；
- 挑后场（最容易处理，所以接发方除非被骗很少这样回，相当于把主动权交回去）。

所以发完小球，要站在上述发球位置不要乱动，重心微微下沉，眼睛盯接发方拍面变化做预判，再按 3.0 节"威胁最大的球能到"的原则决定第一步往哪。

**发了高远（或平高）之后**
本质就退化成"打完高远后回中"的情况——参考 3.0 节，站位略偏后，防杀 > 吊 > 平高。
...
底层逻辑参见 3.0 节，本章是其在具体球路下的落地。发球与接发的位置选择见 5.1.1。